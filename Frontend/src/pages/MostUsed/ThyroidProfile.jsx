import React from "react";
import {
  FaArrowLeft,
  FaHeartbeat,
  FaUserMd,
  FaCalendarAlt,
  FaDownload,
  FaPrint,
} from "react-icons/fa";
import { Link } from "react-router-dom";



const Stat = ({ label, value, hint }) => (
  <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
    <div className="flex items-center justify-between">
      <div>
        <p className="text-sm text-gray-500">{label}</p>
        <p className="mt-2 text-xl font-semibold text-gray-900">{value}</p>
      </div>
      {hint && <p className="text-xs text-gray-400">{hint}</p>}
    </div>
  </div>
);

const RiskMeter = ({ percent = 12 }) => {
  const clamp = Math.max(0, Math.min(100, percent));
  const hue = 120 - Math.round((clamp / 100) * 120);
  return (
    <div aria-hidden className="w-full">
      <div className="flex items-center justify-between mb-2">
        <p className="text-sm font-medium text-gray-700">Estimated endocrine risk</p>
        <p className="text-sm font-semibold text-gray-900">{clamp}%</p>
      </div>
      <div className="w-full h-3 rounded-full bg-gray-100 overflow-hidden">
        <div
          className="h-full transition-all duration-700"
          style={{
            width: `${clamp}%`,
            background: `linear-gradient(90deg, hsl(${hue} 70% 45%), hsl(${Math.max(
              0,
              hue - 20
            )} 70% 40%))`,
          }}
        />
      </div>
      <div className="mt-3 text-xs text-gray-500 flex gap-3">
        <span className="inline-flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-green-500" />
          Low
        </span>
        <span className="inline-flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-yellow-400" />
          Moderate
        </span>
        <span className="inline-flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-red-500" />
          High
        </span>
      </div>
    </div>
  );
};

export default function ThyroidProfile() {
  const testDetails = {
    whatIs:
      "A thyroid profile evaluates thyroid gland function by measuring hormones (TSH, Free T4, Free T3) and antibodies when indicated. It helps diagnose hypo- and hyperthyroid states and guides treatment and monitoring.",
    whyImportant: [
      "Thyroid disorders affect ~12% of the population across lifespans.",
      "Undiagnosed hypothyroidism increases cardiovascular risk and impairs quality of life.",
      "Hyperthyroidism accelerates bone loss and may cause cardiac arrhythmias.",
      "Thyroid health is critical for pregnancy, fertility, and metabolic regulation.",
    ],
    testsIncluded: [
      "TSH — Primary screening (Normal ~0.4–4.0 mIU/L)",
      "Free T4 — Active thyroid hormone (Normal ~0.8–1.8 ng/dL)",
      "Free T3 — Active thyroid hormone (Normal ~2.3–4.2 pg/mL)",
      "Thyroid Antibodies — Detect autoimmune thyroid disease (TPOAb, TgAb)",
    ],
    symptoms: {
      hypothyroidism: [
        "Fatigue",
        "Weight gain",
        "Cold intolerance",
        "Dry skin",
        "Constipation",
        "Depression",
      ],
      hyperthyroidism: [
        "Weight loss",
        "Rapid heartbeat",
        "Anxiety",
        "Tremors",
        "Heat intolerance",
        "Sleep problems",
      ],
    },
    whenToTest: [
      "Unexplained weight changes or energy shifts",
      "Menstrual irregularities or infertility concerns",
      "Planning pregnancy or during prenatal care",
      "History of autoimmune disease or neck irradiation",
      "Unexplained high cholesterol or persistent symptoms",
    ],
  };

  const estimatedRisk = 12; 

  return (
    <div className="min-h-screen bg-white text-gray-900">
      {}
      <div className="sticky top-5 z-40 px-4">
        <div className="max-w-7xl mx-auto bg-white/90 backdrop-blur-md border border-gray-100 rounded-2xl shadow-sm py-3 px-5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Link
              to="/tests"
              className="inline-flex items-center gap-2 text-sm text-gray-700 hover:text-indigo-600 transition"
              aria-label="Back to tests"
            >
              <FaArrowLeft />
              Back
            </Link>
            <div className="hidden sm:flex items-center gap-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 text-sm font-semibold shadow-sm">
                <FaUserMd /> Thyroid Profile
              </div>
              <div className="text-sm text-gray-500">Comprehensive endocrine panel</div>
            </div>
          </div>

          <div className="text-center">
            <h1 className="text-lg sm:text-xl font-extrabold tracking-tight">Thyroid Profile</h1>
            <p className="text-xs text-gray-500">TSH · Free T4 · Free T3 · Antibodies</p>
          </div>

          <div className="flex items-center gap-3">
            <button className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-600 text-white font-semibold shadow hover:bg-indigo-700 transition">
              Book This Test
            </button>
            <button
              className="inline-flex items-center gap-2 px-3 py-2 rounded-full border border-gray-100 bg-white text-gray-700 hover:shadow transition"
              aria-label="Download PDF"
            >
              <FaDownload />
              Download
            </button>
            <button
              className="inline-flex items-center gap-2 px-3 py-2 rounded-full border border-gray-100 bg-white text-gray-700 hover:shadow transition"
              aria-label="Print"
              onClick={() => window.print()}
            >
              <FaPrint />
              Print
            </button>
          </div>
        </div>
      </div>

      {}
      <section className="mt-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="rounded-3xl overflow-hidden grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
            <div className="lg:col-span-2 bg-gradient-to-br from-indigo-50 to-white p-8 sm:p-12 flex flex-col justify-between border border-gray-100">
              <div>
                <div className="inline-flex items-center gap-3 rounded-full bg-white shadow px-3 py-1 border border-gray-100 mb-4 w-max">
                  <FaHeartbeat className="text-indigo-600" />
                  <span className="text-sm font-medium text-gray-700">Clinically validated</span>
                </div>

                <h2 className="text-3xl font-extrabold text-gray-900 mb-3">Complete Thyroid Evaluation</h2>
                <p className="text-lg text-gray-700 mb-6">{testDetails.whatIs}</p>

                <div className="grid sm:grid-cols-2 gap-4">
                  {testDetails.testsIncluded.map((c, i) => {
                    const [title, rest] = c.split(":");
                    return (
                      <div key={i} className="rounded-xl bg-white border border-gray-100 p-4 shadow-sm">
                        <p className="text-sm font-semibold text-gray-900">{title}</p>
                        <p className="mt-1 text-sm text-gray-600">{rest?.trim()}</p>
                      </div>
                    );
                  })}
                </div>
              </div>

              <div className="mt-8 flex flex-col sm:flex-row gap-3 items-stretch">
                <button className="flex-1 rounded-xl px-5 py-3 bg-indigo-600 text-white font-semibold shadow hover:bg-indigo-700 transition">
                  Book Now — $11.75
                </button>

                <Link
                  to="/consult-endocrinologist"
                  className="flex-1 rounded-xl px-5 py-3 border border-gray-100 bg-white text-gray-800 text-center shadow-sm hover:shadow transition flex items-center justify-center gap-2"
                >
                  Consult an Endocrinologist
                </Link>
              </div>
            </div>

            <aside className="bg-white border border-gray-100 p-6 sm:p-8 rounded-2xl shadow-sm flex flex-col gap-6">
              <div>
                <h3 className="text-sm font-semibold text-gray-500">Snapshot</h3>
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <Stat label="Results Time" value="24–48 hrs" />
                  <Stat label="Sample" value="Blood (Venous)" />
                </div>
              </div>

              <div>
                <RiskMeter percent={estimatedRisk} />
              </div>

              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2">When to test</h4>
                <ul className="text-sm text-gray-600 space-y-2">
                  {testDetails.whenToTest.slice(0, 3).map((w, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <span className="mt-1 inline-block w-2.5 h-2.5 rounded-full bg-indigo-500" />
                      <span>{w}</span>
                    </li>
                  ))}
                </ul>
                <Link to="/when-to-test" className="mt-3 inline-block text-sm text-indigo-600 hover:underline">
                  Learn more about timing
                </Link>
              </div>
            </aside>
          </div>
        </div>
      </section>

      {}
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <article className="lg:col-span-2 space-y-8">
          <section className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-900 mb-3 flex items-center gap-3">
              <FaHeartbeat className="text-indigo-600" />
              What the numbers mean
            </h3>
            <p className="text-gray-700 mb-4">
              Results are interpreted in context — age, pregnancy, medications and comorbidities matter. Your clinician will correlate TSH with Free T4/Free T3 and antibodies for diagnosis and management.
            </p>

            <div className="grid sm:grid-cols-3 gap-4">
              <div className="rounded-lg p-4 bg-gradient-to-b from-white to-gray-50 border border-gray-100">
                <p className="text-sm text-gray-500">TSH</p>
                <p className="mt-2 font-semibold text-gray-900">Normal ~0.4–4.0 mIU/L</p>
              </div>

              <div className="rounded-lg p-4 bg-gradient-to-b from-white to-gray-50 border border-gray-100">
                <p className="text-sm text-gray-500">Free T4</p>
                <p className="mt-2 font-semibold text-gray-900">Normal ~0.8–1.8 ng/dL</p>
              </div>

              <div className="rounded-lg p-4 bg-gradient-to-b from-white to-gray-50 border border-gray-100">
                <p className="text-sm text-gray-500">Free T3</p>
                <p className="mt-2 font-semibold text-gray-900">Normal ~2.3–4.2 pg/mL</p>
              </div>
            </div>
          </section>

          <section className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-900 mb-3">Why this matters</h3>
            <ul className="list-disc list-inside space-y-3 text-gray-700">
              {testDetails.whyImportant.map((w, i) => (
                <li key={i}>{w}</li>
              ))}
            </ul>
          </section>

          <section className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
            <h3 className="text-xl font-semibold text-gray-900 mb-3">Symptoms to watch for</h3>
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="rounded-lg p-4 border border-gray-100">
                <h4 className="font-semibold text-gray-800 mb-2">Hypothyroidism</h4>
                <ul className="text-gray-700 space-y-2">
                  {testDetails.symptoms.hypothyroidism.map((s, idx) => (
                    <li key={idx}>{s}</li>
                  ))}
                </ul>
              </div>

              <div className="rounded-lg p-4 border border-gray-100">
                <h4 className="font-semibold text-gray-800 mb-2">Hyperthyroidism</h4>
                <ul className="text-gray-700 space-y-2">
                  {testDetails.symptoms.hyperthyroidism.map((s, idx) => (
                    <li key={idx}>{s}</li>
                  ))}
                </ul>
              </div>
            </div>
          </section>
        </article>

        {}
        <aside className="lg:col-span-1 space-y-6 sticky top-28 self-start">
          <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
            <h4 className="text-sm font-semibold text-gray-700 mb-3">Need help?</h4>
            <p className="text-sm text-gray-600 mb-4">Schedule a consult or chat with our specialists for personalized interpretation.</p>
            <div className="flex flex-col gap-3">
              <Link
                to="/consult"
                className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition"
              >
                Schedule consult
              </Link>
              <Link
                to="/chat"
                className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full border border-gray-100 bg-white text-gray-800 hover:shadow transition"
              >
                Chat with support
              </Link>
            </div>
          </div>

          <div className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm">
            <h4 className="text-sm font-semibold text-gray-700 mb-3">Printable summary</h4>
            <p className="text-sm text-gray-600 mb-4">Download or print a summary to take to your clinician.</p>
            <div className="flex gap-3">
              <button onClick={() => window.print()} className="flex-1 rounded-md px-3 py-2 border border-gray-100 bg-white text-gray-800">
                Print
              </button>
              <button className="flex-1 rounded-md px-3 py-2 bg-indigo-600 text-white">
                Download PDF
              </button>
            </div>
          </div>
        </aside>
      </main>

      <footer className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <div className="rounded-xl border border-gray-100 bg-white p-6 text-sm text-gray-600 text-center shadow-sm">
          <p>
            Results are for informational purposes only. Always consult a clinician for diagnosis and treatment. If you experience severe symptoms, seek immediate medical care.
          </p>
        </div>
      </footer>
    </div>
  );
}